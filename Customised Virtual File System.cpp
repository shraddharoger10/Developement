...
void CreateDILB()
{
	...
	while(i<= MAXINODE)    //Singly LinearLinkedList which contains 50 Inode
	{
...
int OpenFile(char *name,int mode)
{
	...
	else if(mode == READ)     //if we open file in read mode
	{
		UFDTArr[i].ptrfiletable->readoffset = 0;
	}
	else if(mode == WRITE)    // if we open file in write mode
	{
		UFDTArr[i].ptrfiletable->writeoffset = 0;
	}
	UFDTArr[i].ptrfiletable->ptrinode = temp;
